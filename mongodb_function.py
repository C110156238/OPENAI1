...
client = pymongo.MongoClient("mongodb+srv://xiapapapa:xiapapapa@cluster0.zsqsgiq.mongodb.net/?retryWrites=true&w=majority")
...
def read_all_message():
    """讀取所有訊息內容"""
    messages = list(client.MongoClient.Database.find())
    return messages
...
def get_message(_id: str) -> list:
    query = client.MongoClient.Database.find_one(
        {
            '_id': ObjectId(_id)
        }
    )
...
def update_message(_id: str, message: dict):
    client.MongoClient.Database.update_one(
        {
            '_id': ObjectId(_id)
        },{
            '$set': message
